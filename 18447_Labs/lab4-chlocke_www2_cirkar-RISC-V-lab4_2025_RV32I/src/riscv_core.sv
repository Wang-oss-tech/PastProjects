...
module riscv_core (
    ...
    /* Branching Strategy For Each Prediction Scheme
        2'd0 = predict always not taken
        2'd1 = 1-bit counter (Branch History State Machine)
        2'd2 = 2 bit hysterisis
    */
    
    logic incInstrStall, instr_stallTemp, incInstrStall1;
    always_ff @(posedge clk) begin
        if (incInstrStall === 1'b0) incInstrStall1 <= 1'b0;
        else incInstrStall1 <= 1'b1;
    end

    // stall if instr_stall is asserted (normal stall for load-use hazards/ ecall)
    // incInstrStall & incInstrStall1 de-asserts instr_stall if you are stalled bc
    // of an instruction fetched while writing back to the cache (to replace 
    // instr in fetch stage)
    assign instr_stall = instr_stallTemp & incInstrStall & incInstrStall1;
    ...
    logic prev_instr_stalled;//, prev_instr_stalled2;
    logic [2:00] numStalls;
    logic [2:0] prevNumStalls;
    logic [3:0] forwardA, forwardB; // forwarding control signals
    ...
    logic [3:0] forwardStore;

    // New Lab4 logic vars for additional mem stage regs
    logic [31:0] npc_plus4_F1F2DEM1M2M3, npc_plus4_F1F2DEM1M2M3M4, npc_plus4_F1F2DEM1M2M3M4M5, npc_plus4_F1F2DEM1M2M3M4M5M6, npc_plus4_F1F2DEM1M2M3M4M5M6M7, npc_plus4_F1F2DEM1M2M3M4M5M6M7M8;
    logic [31:0] instr_F2DEM1M2M3, instr_F2DEM1M2M3M4, instr_F2DEM1M2M3M4M5, instr_F2DEM1M2M3M4M5M6, instr_F2DEM1M2M3M4M5M6M7, instr_F2DEM1M2M3M4M5M6M7M8;
    logic [31:0] alu_out_EM1M2M3, alu_out_EM1M2M3M4, alu_out_EM1M2M3M4M5, alu_out_EM1M2M3M4M5M6, alu_out_EM1M2M3M4M5M6M7, alu_out_EM1M2M3M4M5M6M7M8;
    //M3, data_load_temp_EM1M2M3M4, data_load_temp_EM1M2M3M4M5, data_load_temp_EM1M2M3M4M5M6, data_load_temp_EM1M2M3M4M5M6M7, data_load_temp_EM1M2M3M4M5M6M7M8;
    logic [31:0] rs1_data_DEM1M2M3, rs1_data_DEM1M2M3M4, rs1_data_DEM1M2M3M4M5, rs1_data_DEM1M2M3M4M5M6, rs1_data_DEM1M2M3M4M5M6M7, rs1_data_DEM1M2M3M4M5M6M7M8;
    ctrl_signals_t ctrl_signals_DEM1M2M3, ctrl_signals_DEM1M2M3M4, ctrl_signals_DEM1M2M3M4M5, ctrl_signals_DEM1M2M3M4M5M6, ctrl_signals_DEM1M2M3M4M5M6M7, ctrl_signals_DEM1M2M3M4M5M6M7M8;
    logic [4:0] rs1_DEM1M2M3, rs1_DEM1M2M3M4, rs1_DEM1M2M3M4M5, rs1_DEM1M2M3M4M5M6, rs1_DEM1M2M3M4M5M6M7, rs1_DEM1M2M3M4M5M6M7M8;
    logic flushed_DEM1M2M3, flushed_DEM1M2M3M4, flushed_DEM1M2M3M4M5, flushed_DEM1M2M3M4M5M6, flushed_DEM1M2M3M4M5M6M7, flushed_DEM1M2M3M4M5M6M7M8;

    // Lab 4- Cache logic
    logic cache_read_hit, cache_read_hit_M1, cache_read_hit_M1M2, 
        cache_read_hit_M1M2M3, cache_read_hit_M1M2M3M4, 
        cache_read_hit_M1M2M3M4M5, cache_read_hit_M1M2M3M4M5M6, 
        cache_read_hit_M1M2M3M4M5M6M7, cache_read_hit_M1M2M3M4M5M6M7M8;
    logic [31:0] cache_read_data, cache_read_data_M1, cache_read_data_M1M2, 
        cache_read_data_M1M2M3, cache_read_data_M1M2M3M4, 
        cache_read_data_M1M2M3M4M5, cache_read_data_M1M2M3M4M5M6, 
        cache_read_data_M1M2M3M4M5M6M7, cache_read_data_M1M2M3M4M5M6M7M8;
    logic stallForCacheMiss, sourcedFromMainMem;
    // global signals to determine count values
    logic cache_rd_wr, cache_read_miss, cache_is_eviction, cache_enable;
    ...
    // Counters for Lab 4
    logic [31:0] num_total_cache_reads;
    logic [31:0] num_cache_read_misses;
    logic [31:0] num_cache_read_hits;
    logic [31:0] num_cache_writes;
    logic [31:0] num_writes_eviction;
    ...
    always_ff @(posedge clk, negedge rst_l) begin
        if (~rst_l) begin
            ...
            prev_instr_stalled <= instr_stallTemp;
            if (~prev_instr_stalled && instr_stallTemp) 
                num_stall_instr_ID <= num_stall_instr_ID + 32'd1;
            if (instr_stallTemp) numStalls <= numStalls + 3'd1;
            else numStalls <= 3'd0;
            prevNumStalls <= numStalls;
            if (prev_instr_stalled & ~instr_stallTemp) begin
    ...
    // additional logic to keep track of whether the last/second to last instr 
    // was stalled for a cache miss in order to save the instr missed from 
    // cacheStalling
    logic stallForCacheMiss1;//, stallForCacheMiss2;
    logic [31:0] firstInstrMissed;//, secInstrMissed;
    always_ff @(posedge clk) begin
        if (~rst_l) begin
            stallForCacheMiss1 <= 1'b0;
            //stallForCacheMiss2 <= 1'b0;
        end else begin
            stallForCacheMiss1 <= stallForCacheMiss;
            //stallForCacheMiss2 <= stallForCacheMiss1;
        end

        if (stallForCacheMiss & !stallForCacheMiss1) firstInstrMissed <= instr;
        //if (stallForCacheMiss1 & !stallForCacheMiss2) secInstrMissed <= instr;
    end

    logic [3:0] cacheStallCounter, last_cacheStallCounter;
    
    /*
    // Cache counter logic
    always_ff @(posedge clk, negedge rst_l) begin
        if (~rst_l) begin
            num_total_cache_reads <= 'd0;
            num_cache_read_misses <= 'd0;
            num_cache_read_hits <= 'd0;
            num_cache_writes <= 'd0;
            num_writes_eviction <= 'd0;
        end
        else begin
            if (cache_rd_wr & cache_enable & (cache_read_hit | 
                (cache_read_miss & (cacheStallCounter === 4'd1)))) begin
                num_total_cache_reads <= num_total_cache_reads + 'd1;
            end
            if (cache_read_miss & (cacheStallCounter === 4'd1)) begin
                num_cache_read_misses <= num_cache_read_misses + 'd1;
            end
            if (cache_read_hit) begin
                num_cache_read_hits <= num_cache_read_hits + 'd1;
            end
            if (~cache_rd_wr & cache_enable) begin
                num_cache_writes <= num_cache_writes + 'd1;
            end
            if (cache_is_eviction) begin
                num_writes_eviction <= num_writes_eviction + 'd1;
            end
        end
    end*/
    ...
    registerF1F2 r1 (
        .clk, 
        .en(~instr_stallTemp & ~stallForCacheMiss), 
        .pc, 
        .npc_plus4, 
        .pc_F1, 
        .npc_plus4_F1, 
    ...
    always_ff @(posedge clk) begin
        ...
        else lastInstrStall <= instr_stallTemp;
    end
...
/*
    always_ff @(posedge clk) begin
        //if (needToFlush) lastInstrFlushed <= 1'b1;
        //else lastInstrFlushed <= 1'b0;
        if (instr_stallTemp) prev_instr_stalled2 <= 1'b1;
        else prev_instr_stalled2 <= 1'b0;
    end*/

    always_comb begin
        instrToProp = (instr_stallTemp) ? 32'd0: instr[0];
    end
    ...
    execute executeStage(
        ...
        .alu_out_EM1M2M3M4,
        .alu_out_EM1M2M3M4M5,
        .alu_out_EM1M2M3M4M5M6,
        .alu_out_EM1M2M3M4M5M6M7,
        .alu_out(alu_out),
        .npc_jal, 
        .npc_offset, 
        .pc_indirect,
        ...
        .instr_F2DEM1M2M3,
        .instr_F2DEM1M2M3M4,
        .instr_F2DEM1M2M3M4M5,
        .instr_F2DEM1M2M3M4M5M6,
        .instr_F2DEM1M2M3M4M5M6M7,
        .npc_plus4_F1F2DEM1,
        .next_pc_F1F2D,
        .npc_plus4_F1F2DEM1M2,
        .npc_plus4_F1F2DEM1M2M3,
        .npc_plus4_F1F2DEM1M2M3M4,
        .npc_plus4_F1F2DEM1M2M3M4M5,
        .npc_plus4_F1F2DEM1M2M3M4M5M6,
        .npc_plus4_F1F2DEM1M2M3M4M5M6M7,
        //.branch_is_forward_out,
        .cache_read_data_M1,
        .cache_read_data_M1M2,
        .cache_read_data_M1M2M3,
        .cache_read_data_M1M2M3M4,
        .cache_read_data_M1M2M3M4M5,
        .cache_read_data_M1M2M3M4M5M6,
        .cache_read_data_M1M2M3M4M5M6M7,
        .stallForCacheMiss,
        //.stallForCacheMiss1,
        .instr_stallTemp
    );
    ...
    registerM3M4 r7 (
        ...
        .alu_out_EM1M2M3,
        .rs1_data_DEM1M2M3,
        .ctrl_signals_DEM1M2M3,
        .rs1_DEM1M2M3,
        .flushed_DEM1M2,
        .flushed_DEM1M2M3,
        .cache_read_hit_M1M2,
        .cache_read_data_M1M2,
        .cache_read_hit_M1M2M3,
        .cache_read_data_M1M2M3
    );
    ...
    registerM4M5 r8 (
        ...
        .alu_out_EM1M2M3M4,
        .rs1_data_DEM1M2M3M4,
        .ctrl_signals_DEM1M2M3M4,
        .rs1_DEM1M2M3M4,
        .flushed_DEM1M2M3,
        .flushed_DEM1M2M3M4,
        .cache_read_hit_M1M2M3,
        .cache_read_data_M1M2M3,
        .cache_read_hit_M1M2M3M4,
        .cache_read_data_M1M2M3M4
    );
    ...
    registerM5M6 r9 (
        ...
        .instr_F2DEM1M2M3M4M5,
        .alu_out_EM1M2M3M4M5,
        .rs1_data_DEM1M2M3M4M5,
        .ctrl_signals_DEM1M2M3M4M5,
        .rs1_DEM1M2M3M4M5,
        .flushed_DEM1M2M3M4,
        .flushed_DEM1M2M3M4M5,
        .cache_read_hit_M1M2M3M4,
        .cache_read_data_M1M2M3M4,
        .cache_read_hit_M1M2M3M4M5,
        .cache_read_data_M1M2M3M4M5
    );
    ...
    registerM6M7 r10 (
        ...
        .instr_F2DEM1M2M3M4M5M6,
        .alu_out_EM1M2M3M4M5M6,
        .rs1_data_DEM1M2M3M4M5M6,
        .ctrl_signals_DEM1M2M3M4M5M6,
        .rs1_DEM1M2M3M4M5M6,
        .flushed_DEM1M2M3M4M5,
        .flushed_DEM1M2M3M4M5M6,
        .cache_read_hit_M1M2M3M4M5,
        .cache_read_data_M1M2M3M4M5,
        .cache_read_hit_M1M2M3M4M5M6,
        .cache_read_data_M1M2M3M4M5M6
    );

    // Pipeline register between MEMORY7 and MEMORY8
    registerM7M8 r11 (
        .clk,
        .en(1'b1),
        .npc_plus4_F1F2DEM1M2M3M4M5M6,
        ...
        .instr_F2DEM1M2M3M4M5M6M7,
        .alu_out_EM1M2M3M4M5M6M7,
        .rs1_data_DEM1M2M3M4M5M6M7,
        .ctrl_signals_DEM1M2M3M4M5M6M7,
        .rs1_DEM1M2M3M4M5M6M7,
        .flushed_DEM1M2M3M4M5M6,
        .flushed_DEM1M2M3M4M5M6M7,
        .cache_read_hit_M1M2M3M4M5M6,
        .cache_read_data_M1M2M3M4M5M6,
        .cache_read_hit_M1M2M3M4M5M6M7,
        .cache_read_data_M1M2M3M4M5M6M7
    );

    // Pipeline register between MEMORY8 and WRITEBACK
    registerM8W r12 (
        .clk,
        .en(1'b1),
        .npc_plus4_F1F2DEM1M2M3M4M5M6M7,
        ...
        .ctrl_signals_DEM1M2M3M4M5M6M7,
        .rs1_DEM1M2M3M4M5M6M7,
        .npc_plus4_F1F2DEM1M2M3M4M5M6M7M8,
        .instr_F2DEM1M2M3M4M5M6M7M8,
        .alu_out_EM1M2M3M4M5M6M7M8,
        .rs1_data_DEM1M2M3M4M5M6M7M8,
        .ctrl_signals_DEM1M2M3M4M5M6M7M8,
        .rs1_DEM1M2M3M4M5M6M7M8,
        .flushed_DEM1M2M3M4M5M6M7,
        .flushed_DEM1M2M3M4M5M6M7M8,
        .cache_read_hit_M1M2M3M4M5M6M7,
        .cache_read_data_M1M2M3M4M5M6M7,
        .cache_read_hit_M1M2M3M4M5M6M7M8,
        .cache_read_data_M1M2M3M4M5M6M7M8
    );

    // signal to alert the cache that the data has been loaded from main memory 
    // and is ready to be written to the cache if a miss occurred
    always_comb begin
        sourcedFromMainMem = (instr_F2DE == instr_F2DEM1M2M3M4M5M6M7M8) & 
            (cacheStallCounter === 4'd8) & (last_cacheStallCounter === 4'd7);
    end
    ...
    writeback writebackStage (
        //.clk, 
        //.rst_l, 
        .ctrl_signals(ctrl_signals_DEM1M2M3M4M5M6M7M8),
        .npc_plus4(npc_plus4_F1F2DEM1M2M3M4M5M6M7M8), 
        .alu_out(alu_out_EM1M2M3M4M5M6M7M8), 
        .instr(instr_F2DEM1M2M3M4M5M6M7M8), 
        .rd_data, 
        .timeToWrite, 
        .rdToWrite, 
        .rs1(rs1_DEM1M2M3M4M5M6M7M8), 
        .rs1_data(rs1_data_DEM1M2M3M4M5M6M7M8), 
        .syscall_halt, 
        .data_load(data_load[0]),
        .cache_hit(cache_read_hit_M1M2M3M4M5M6M7M8),
        .cache_read_data(cache_read_data_M1M2M3M4M5M6M7M8)
    );
    ...
    always_comb begin
        ...
        if (ctrl_signals_D.memWrite & !ctrl_signals_DE.memWrite & 
            ctrl_signals_DE.rfWrite & (instr_F2DE[11:7] == instr_F2D[24:20]) & 
            (forwardB == 4'd0) & !flushed_DE) begin
            forwardStore = 4'b0010; // Forwarding for store instructions from M1 to Ex
        end
        else if (ctrl_signals_D.memWrite & !ctrl_signals_DEM1.memWrite & 
            ctrl_signals_DEM1.rfWrite & (instr_F2DEM1[11:7] == instr_F2D[24:20]) & 
            (forwardB == 4'd0) & !flushed_DEM1) begin
            forwardStore = 4'b0001; // Forwarding for store instructions from M2 to Ex
        end
        else if (ctrl_signals_D.memWrite & !ctrl_signals_DEM1M2.memWrite & 
            ctrl_signals_DEM1M2.rfWrite & (instr_F2DEM1M2[11:7] == instr_F2D[24:20]) & 
            (forwardB == 4'd0) & !flushed_DEM1M2) begin
            forwardStore = 4'b0011; // Forwarding for store instructions from M3 to Ex
        end
        else if (ctrl_signals_D.memWrite & !ctrl_signals_DEM1M2M3.memWrite & 
            ctrl_signals_DEM1M2M3.rfWrite & (instr_F2DEM1M2M3[11:7] == instr_F2D[24:20]) & 
            (forwardB == 4'd0) & !flushed_DEM1M2M3) begin
            forwardStore = 4'd4; // Forwarding for store instructions from M4 to Ex
        end
        else if (ctrl_signals_D.memWrite & !ctrl_signals_DEM1M2M3M4.memWrite & 
            ctrl_signals_DEM1M2M3M4.rfWrite & (instr_F2DEM1M2M3M4[11:7] == instr_F2D[24:20]) & 
            (forwardB == 4'd0) & !flushed_DEM1M2M3M4) begin
            forwardStore = 4'd5; // Forwarding for store instructions from M5 to Ex
        end
        else if (ctrl_signals_D.memWrite & !ctrl_signals_DEM1M2M3M4M5.memWrite & 
            ctrl_signals_DEM1M2M3M4M5.rfWrite & (instr_F2DEM1M2M3M4M5[11:7] == instr_F2D[24:20]) & 
            (forwardB == 4'd0) & !flushed_DEM1M2M3M4M5) begin
            forwardStore = 4'd6; // Forwarding for store instructions from M6 to Ex
        end
        else if (ctrl_signals_D.memWrite & !ctrl_signals_DEM1M2M3M4M5M6.memWrite & 
            ctrl_signals_DEM1M2M3M4M5M6.rfWrite & (instr_F2DEM1M2M3M4M5M6[11:7] == instr_F2D[24:20]) & 
            (forwardB == 4'd0) & !flushed_DEM1M2M3M4M5M6) begin
            forwardStore = 4'd7; // Forwarding for store instructions from M7 to Ex
        end
        else if (ctrl_signals_D.memWrite & !ctrl_signals_DEM1M2M3M4M5M6M7.memWrite & 
            ctrl_signals_DEM1M2M3M4M5M6M7.rfWrite & (instr_F2DEM1M2M3M4M5M6M7[11:7] == instr_F2D[24:20]) & 
            (forwardB == 4'd0) & !flushed_DEM1M2M3M4M5M6M7) begin
            forwardStore = 4'd8; // Forwarding for store instructions from M8 to Ex
        end
        else if (ctrl_signals_D.memWrite & !ctrl_signals_DEM1M2M3M4M5M6M7M8.memWrite 
            & ctrl_signals_DEM1M2M3M4M5M6M7M8.rfWrite &
            (instr_F2DEM1M2M3M4M5M6M7M8[11:7] == instr_F2D[24:20]) & (forwardB == 4'd0) & 
            !flushed_DEM1M2M3M4M5M6M7M8) begin
            forwardStore = 4'd9; // Forwarding for store instructions from M9 to Ex
        end
        else forwardStore = 4'd0;

        // MEM2 Hazard: Forward from MEM2 stage (MEM2 to EX)
        if (ctrl_signals_DEM1.rfWrite && (instr_F2DEM1[11:7] != 0)) begin
            if (instr_F2DEM1[11:7] == rs1_D & (forwardA == 4'd0)) begin
                forwardA = 4'b0001;
            end
            if (instr_F2DEM1[11:7] == instr_F2D[24:20] && 
                !ctrl_signals_D.useImm & (forwardB == 4'd0)) begin
                forwardB = 4'b0001;
            end
        end


        // MEM3 Hazard: Forward from MEM3 stage (MEM3 to EX)
        if (ctrl_signals_DEM1M2.rfWrite & (instr_F2DEM1M2[11:7] != 0)) begin
            if (instr_F2DEM1M2[11:7] == rs1_D & (forwardA == 4'd0)) begin 
                forwardA = 4'b0011;
            end
        ...
        // MEM4 Hazard: Forward from MEM4 stage (MEM4 to EX)
        if (ctrl_signals_DEM1M2M3.rfWrite & (instr_F2DEM1M2M3[11:7] != 0)) begin
            if (instr_F2DEM1M2M3[11:7] == rs1_D & (forwardA == 4'd0)) begin 
                forwardA = 4'd4;
            end
            if (instr_F2DEM1M2M3[11:7] == instr_F2D[24:20] & 
                !ctrl_signals_D.useImm & (forwardB == 4'd0)) begin
                forwardB = 4'd4;
            end
        end

        // MEM5 Hazard: Forward from MEM5 stage (MEM5 to EX)
        if (ctrl_signals_DEM1M2M3M4.rfWrite & (instr_F2DEM1M2M3M4[11:7] != 0)) begin
            if (instr_F2DEM1M2M3M4[11:7] == rs1_D & (forwardA == 4'd0)) begin 
                forwardA = 4'd5;
            end
            if (instr_F2DEM1M2M3M4[11:7] == instr_F2D[24:20] & 
                !ctrl_signals_D.useImm & (forwardB == 4'd0)) begin
                forwardB = 4'd5;
            end
        end

        // MEM6 Hazard: Forward from MEM6 stage (MEM6 to EX)
        if (ctrl_signals_DEM1M2M3M4M5.rfWrite & (instr_F2DEM1M2M3M4M5[11:7] != 0)) begin
            if (instr_F2DEM1M2M3M4M5[11:7] == rs1_D & (forwardA == 4'd0)) begin 
                forwardA = 4'd6;
            end
            if (instr_F2DEM1M2M3M4M5[11:7] == instr_F2D[24:20] & 
                !ctrl_signals_D.useImm & (forwardB == 4'd0)) begin
                forwardB = 4'd6;
            end
        end

        // MEM7 Hazard: Forward from MEM7 stage (MEM7 to EX)
        if (ctrl_signals_DEM1M2M3M4M5M6.rfWrite & (instr_F2DEM1M2M3M4M5M6[11:7] != 0)) begin
            if (instr_F2DEM1M2M3M4M5M6[11:7] == rs1_D & (forwardA == 4'd0)) begin 
                forwardA = 4'd7;
            end
            if (instr_F2DEM1M2M3M4M5M6[11:7] == instr_F2D[24:20] & 
                !ctrl_signals_D.useImm & (forwardB == 4'd0)) begin
                forwardB = 4'd7;
            end
        end

        // MEM8 Hazard: Forward from MEM8 stage (MEM8 to EX)
        if (ctrl_signals_DEM1M2M3M4M5M6M7.rfWrite & (instr_F2DEM1M2M3M4M5M6M7[11:7] != 0)) begin
            if (instr_F2DEM1M2M3M4M5M6M7[11:7] == rs1_D & (forwardA == 4'd0)) begin 
                forwardA = 4'd8;
            end
            if (instr_F2DEM1M2M3M4M5M6M7[11:7] == instr_F2D[24:20] & 
                !ctrl_signals_D.useImm & (forwardB == 4'd0)) begin
                forwardB = 4'd8;
            end
        end

        // MEM9 Hazard: Forward from MEM9 stage (MEM9 to EX)
        if (ctrl_signals_DEM1M2M3M4M5M6M7M8.rfWrite & (instr_F2DEM1M2M3M4M5M6M7M8[11:7] != 0)) begin
            if (instr_F2DEM1M2M3M4M5M6M7M8[11:7] == rs1_D & (forwardA == 4'd0)) begin 
                forwardA = 4'd9;
            end
            if (instr_F2DEM1M2M3M4M5M6M7M8[11:7] == instr_F2D[24:20] & 
                !ctrl_signals_D.useImm & (forwardB == 4'd0)) begin
                forwardB = 4'd9;
            end
        end
    ...
    always_comb begin
        instr_stallTemp = 1'b0; // Default: No stall

        // Stall if a Load-Use Hazard occurs (Load value not available yet)
        if ((ctrl_signals_DEM1M2M3M4M5M6M7.memRead) &&  // Load instruction in MEM8 stage
            ((rs1 == instr_F2DEM1M2M3M4M5M6M7[11:7]) || (rs2 == instr_F2DEM1M2M3M4M5M6M7[11:7]))
            && (!cache_read_hit_M1M2M3M4M5M6M7)) begin
            instr_stallTemp = 1'b1;
        end
        else if ((ctrl_signals_DEM1M2M3M4M5M6.memRead) &&  // Load instruction in MEM7 stage
            ((rs1 == instr_F2DEM1M2M3M4M5M6[11:7]) || (rs2 == instr_F2DEM1M2M3M4M5M6[11:7]))
            && (!cache_read_hit_M1M2M3M4M5M6)) begin
            instr_stallTemp = 1'b1;
        end
        else if ((ctrl_signals_DEM1M2M3M4M5.memRead) &&  // Load instruction in MEM6 stage
            ((rs1 == instr_F2DEM1M2M3M4M5[11:7]) || (rs2 == instr_F2DEM1M2M3M4M5[11:7]))
            && (!cache_read_hit_M1M2M3M4M5)) begin
            instr_stallTemp = 1'b1;
        end
        else if ((ctrl_signals_DEM1M2M3M4.memRead) &&  // Load instruction in MEM5 stage
            ((rs1 == instr_F2DEM1M2M3M4[11:7]) || (rs2 == instr_F2DEM1M2M3M4[11:7]))
            && (!cache_read_hit_M1M2M3M4)) begin
            instr_stallTemp = 1'b1;
        end
        else if ((ctrl_signals_DEM1M2M3.memRead) &&  // Load instruction in MEM4 stage
            ((rs1 == instr_F2DEM1M2M3[11:7]) || (rs2 == instr_F2DEM1M2M3[11:7]))
            && (!cache_read_hit_M1M2M3)) begin
            instr_stallTemp = 1'b1;
        end
        else if ((ctrl_signals_DEM1M2.memRead) &&  // Load instruction in MEM3 stage
            ((rs1 == instr_F2DEM1M2[11:7]) || (rs2 == instr_F2DEM1M2[11:7]))
            && (!cache_read_hit_M1M2)) begin
            instr_stallTemp = 1'b1;
        end
        else if ((ctrl_signals_DEM1.memRead) &&  // Load instruction in MEM2 stage
            ((rs1 == instr_F2DEM1[11:7]) || (rs2 == instr_F2DEM1[11:7]))
            && (!cache_read_hit_M1)) begin
            instr_stallTemp = 1'b1;
        end
        else if ((ctrl_signals_DE.memRead) &&  // Load instruction in MEM1 stage
            ((rs1 == instr_F2DE[11:7]) || (rs2 == instr_F2DE[11:7]))
            && (!cache_read_hit)) begin
            instr_stallTemp = 1'b1;
        end
        else if ((ctrl_signals_D.memRead) &&  // Load instruction in EX stage
            ((rs1 == instr_F2D[11:7]) || (rs2 == instr_F2D[11:7]))) begin
            instr_stallTemp = 1'b1;
        end
        // Stall for Ecall
                ...
                (ctrl_signals_DEM1M2M3.rfWrite && instr_F2DEM1M2M3[11:7] == 5'd10) || 
                (ctrl_signals_DEM1M2M3M4.rfWrite && instr_F2DEM1M2M3M4[11:7] == 5'd10) || 
                (ctrl_signals_DEM1M2M3M4M5.rfWrite && instr_F2DEM1M2M3M4M5[11:7] == 5'd10) || 
                (ctrl_signals_DEM1M2M3M4M5M6.rfWrite && instr_F2DEM1M2M3M4M5M6[11:7] == 5'd10) || 
                (ctrl_signals_DEM1M2M3M4M5M6M7.rfWrite && instr_F2DEM1M2M3M4M5M6M7[11:7] == 5'd10) || 
                (ctrl_signals_DEM1M2M3M4M5M6M7M8.rfWrite && instr_F2DEM1M2M3M4M5M6M7M8[11:7] == 5'd10) 
        )) begin
            instr_stallTemp = 1'b1;
        end 
        else if (ctrl_signals_D.syscall | ctrl_signals_DE.syscall | 
            ctrl_signals_DEM1.syscall | ctrl_signals_DEM1M2.syscall | 
            ctrl_signals_DEM1M2M3.syscall | ctrl_signals_DEM1M2M3M4.syscall | 
            ctrl_signals_DEM1M2M3M4M5.syscall | 
            ctrl_signals_DEM1M2M3M4M5M6.syscall | 
            ctrl_signals_DEM1M2M3M4M5M6M7.syscall |
            ctrl_signals_DEM1M2M3M4M5M6M7M8.syscall) begin
            instr_stallTemp = 1'b1;
        end
    end
...
`endif*/
    ...
    assign exception_halt   = instr_mem_excpt | data_mem_excpt | 
        (ctrl_signals.illegal_instr & !stallForCacheMiss);
    logic haltedTemp, haltedTemp2, haltedTemp3, haltedTemp4, haltedTemp5, haltedTemp6, haltedTemp7, haltedTemp8, haltedTemp9, haltedTemp10;
    assign haltedTemp = (rst_l & (syscall_halt | exception_halt));// | (cycle_counter == 32'h0000_0600));
    ...
    always_ff @(posedge clk, negedge rst_l) begin
        if (~rst_l) begin
            ...
            haltedTemp4 <= haltedTemp3;
            haltedTemp5 <= haltedTemp4;
            haltedTemp6 <= haltedTemp5;
            haltedTemp7 <= haltedTemp6;
            haltedTemp8 <= haltedTemp7;
            haltedTemp9 <= haltedTemp8;
            haltedTemp10 <= haltedTemp9;
            if (haltedTemp10 == 'd1)
                halted <= haltedTemp10;
            else 
                halted <= 'd0;
...
final begin
    ...
    // Lab 4 Counters - Cache
    $display("\n6. Cache Statistics:");
    $display("  Number of total cache reads :   %d", 
        num_total_cache_reads);
    $display("  Number of cache read misses :   %d", 
        num_cache_read_misses);
    $display("  Number of cache read hits :   %d", 
        num_cache_read_hits);
    $display("  Number of total cache writes :   %d", 
        num_cache_writes);
    $display("  Number of writes that caused an eviction :   %d", 
        num_writes_eviction);   
...
module IF
    (input logic clk, rst_l, halted, instr_stallTemp, needToFlush, bcond, sramWE, stallForCacheMiss, stallForCacheMiss1, //stallForCacheMiss2,
    ...
    logic [31:0] lastPC, secLastPC, thirdLastPC, pcPostStall;
    ...
    always_ff @(posedge clk, negedge rst_l) begin
        ...
        else if (needToFlush) begin
            pc <= next_pc; // Normal PC update (stall prevents update)
            instr_addr <= next_pc[31:2];
            incInstrStall = 1'b1;
        end
        else if (!halted && !instr_stallTemp & !stallForCacheMiss) begin
            pc <= next_pc; // Normal PC update (stall prevents update)
            instr_addr <= next_pc[31:2];
            incInstrStall = 1'b1;
        end
        else if (cacheStallCounter == 4'd5 & !halted &&/* !instr_stall & */stallForCacheMiss) begin
            pc <= pcPostStall; // Normal PC update (stall prevents update)
            instr_addr <= pcPostStall[31:2];
            incInstrStall = 1'b0;
        end
        else if (cacheStallCounter > 4'd5 & !halted/* && !instr_stall*/ & stallForCacheMiss) begin
            pc <= next_pc; // Normal PC update (stall prevents update)
    ...
    always_ff @(posedge clk) begin
        if (!stallForCacheMiss & !instr_stallTemp) begin
            lastPC <= pc;
            secLastPC <= lastPC;
            thirdLastPC <= secLastPC;
        end
        else begin
            lastPC <= lastPC;
            secLastPC <= secLastPC;
            thirdLastPC <= thirdLastPC;
        end
        if (stallForCacheMiss & !stallForCacheMiss1) begin
            pcPostStall <= secLastPC;
        end
    end

    always_ff @(posedge clk) begin
        last_cacheStallCounter <= cacheStallCounter;
        if (stallForCacheMiss & !stallForCacheMiss1) cacheStallCounter <= 4'd1;
        else if (stallForCacheMiss) cacheStallCounter <= cacheStallCounter + 4'd1;
    end
...
module decode 
    (input logic clk, rst_l, halted, timeToWrite, instr_stallTemp, needToFlush, stallForCacheMiss,
    ...
    riscv_decode Decoder(.rst_l, .stall(instr_stallTemp | stallForCacheMiss), .instr, 
    ...
    assign timeToWrite2 = 
        ((timeToWrite == 'b1) && ^timeToWrite !== 1'bx) ? 1'b1: 1'b0;
    assign match = 
        (((rs1 == rdToWrite) == 'b1) && ^rdToWrite !== 1'bx) ? 1'b1:1'b0;
    assign rs1_data = (match & timeToWrite2 & rdToWrite != 4'd0) ? rd_data: rs1_dataTemp;

    assign timeToWrite22 = 
        ((timeToWrite == 'b1) && ^timeToWrite !== 1'bx) ? 1'b1: 1'b0;
    assign match2 = 
        (((rs2 == rdToWrite) == 'b1) && ^rdToWrite !== 1'bx) ? 1'b1:1'b0;
    assign rs2_data = (match2 & timeToWrite22 & rdToWrite != 4'd0) ? rd_data: rs2_dataTemp;

    // se_immediate generation- based on imm_mode control signal
    always_comb begin
    ...
    // asserts if an instr needs to be flushed after a branch is incorrectly predicted
...
module execute
    (input logic clk,
    input logic bTaken, stallForCacheMiss, 
    input logic instr_stallTemp,
    input logic [1:0] predScheme,
    input logic [3:0] forwardStore,
    ...
    input logic  [31:0] npc_plus4_F1F2DEM1M2, npc_plus4_F1F2DEM1M2M3, 
    input logic  [31:0] npc_plus4_F1F2DEM1M2M3M4, npc_plus4_F1F2DEM1M2M3M4M5, 
    input logic  [31:0] npc_plus4_F1F2DEM1M2M3M4M5M6,
    input logic  [31:0] instr_F2DEM1,  npc_plus4_F1F2DEM1, next_pc_F1F2D,
    input logic  [31:0] instr_F2DEM1M2, instr_F2DEM1M2M3, instr_F2DEM1M2M3M4, 
    input logic  [31:0] instr_F2DEM1M2M3M4M5, instr_F2DEM1M2M3M4M5M6,
    input logic  [31:0] instr_F2DEM1M2M3M4M5M6M7, npc_plus4_F1F2DEM1M2M3M4M5M6M7,
    input ctrl_signals_t  ctrl_signals,
    input logic [3:0] forwardA, forwardB, // Forwarding control signals
    input logic [31:0] alu_out_E, alu_out_EM1, alu_out_EM1M2, // Forwarded values
    input logic [31:0] alu_out_EM1M2M3, alu_out_EM1M2M3M4, alu_out_EM1M2M3M4M5, // Forwarded values
    input logic [31:0] alu_out_EM1M2M3M4M5M6, alu_out_EM1M2M3M4M5M6M7, // Forwarded values
    output logic [31:0] alu_out,
    ...
    input logic [31:0] cache_read_data_M1, cache_read_data_M1M2, 
    input logic [31:0] cache_read_data_M1M2M3, cache_read_data_M1M2M3M4, 
    input logic [31:0] cache_read_data_M1M2M3M4M5, cache_read_data_M1M2M3M4M5M6, 
    input logic [31:0] cache_read_data_M1M2M3M4M5M6M7);

    import RISCV_ISA::*;

    logic [31:0] alu_src2_forwarded, alu_src2;
    logic [31:0] last_alu_src1_forwarded, last_alu_src2_forwarded, last_instr;
    logic [31:0] last_alu_src1_forwarded2, last_last_alu_src1_forwarded2;
    logic [31:0] last_alu_src2_forwarded2, last_last_alu_src2_forwarded2;
    logic [3:0] last_forwardA, last_forwardB;
    logic needToFwdAfterStall, lastNeedToFwdAfterStall;
    logic lastNeedToFwdBfterStall, needToFwdBAfterStall;

    always_ff @(posedge clk) begin
        last_alu_src1_forwarded <= alu_src1_forwarded;
        last_forwardA <= forwardA;
        last_alu_src2_forwarded <= alu_src2_forwarded;
        last_forwardB <= forwardB;
        last_instr <= instr;
        last_last_alu_src1_forwarded2 <= last_alu_src1_forwarded2;

        if ((forwardA == 4'd9) && (stallForCacheMiss || instr_stallTemp)) begin
            last_alu_src1_forwarded2 <= alu_src1_forwarded;
            needToFwdAfterStall      <= 1'b1;
        end
        else if (needToFwdAfterStall===1'b1 && 
            (stallForCacheMiss || instr_stallTemp)) begin
            last_alu_src1_forwarded2 <= last_alu_src1_forwarded2;
            needToFwdAfterStall      <= 1'b1;
        end
        else begin
            last_alu_src1_forwarded2 <= 32'hXXXX_XXXX;
            needToFwdAfterStall      <= 1'b0;
        end

        if (needToFwdAfterStall===1'b1)
            lastNeedToFwdAfterStall <= 1'b1;
        else if (lastNeedToFwdAfterStall===1'b1) begin
            if (!instr_stallTemp && !stallForCacheMiss)
                lastNeedToFwdAfterStall <= 1'b0;
            else
                lastNeedToFwdAfterStall <= 1'b1;
        end
        else begin
        ...
        last_last_alu_src2_forwarded2 <= last_alu_src2_forwarded2;

        if ((forwardB == 4'd9) && (stallForCacheMiss || instr_stallTemp)) begin
            last_alu_src2_forwarded2 <= alu_src2_forwarded;
            needToFwdBAfterStall      <= 1'b1;
        end
        else if (needToFwdAfterStall===1'b1 && 
            (stallForCacheMiss || instr_stallTemp)) begin
            last_alu_src2_forwarded2 <= last_alu_src2_forwarded2;
            needToFwdBAfterStall      <= 1'b1;
        end
        else begin
            last_alu_src2_forwarded2 <= 32'hXXXX_XXXX;
            needToFwdBAfterStall      <= 1'b0;
        end

        if (needToFwdBAfterStall===1'b1)
            lastNeedToFwdBfterStall <= 1'b1;
        else if (lastNeedToFwdBfterStall===1'b1) begin
            if (!instr_stallTemp && !stallForCacheMiss)
                lastNeedToFwdBfterStall <= 1'b0;
            else
                lastNeedToFwdBfterStall <= 1'b1;
        end
        else begin
        ...
        case (forwardStore)
            ...
            4'b0010: begin
                if (opcode_t'(instr_F2DE[6:0]) == OP_JAL | 
                    opcode_t'(instr_F2DE[6:0]) == OP_JALR) begin // jump instr
                    rs2_data_out = npc_plus4_F1F2DE;
                end
                else
                    rs2_data_out = alu_out_E;     // MEM1 forwarding
            end
            4'b0001: begin
                if (opcode_t'(instr_F2DEM1[6:0]) == OP_JAL | 
                    opcode_t'(instr_F2DEM1[6:0]) == OP_JALR) begin // jump instr
                    rs2_data_out = npc_plus4_F1F2DEM1;
                end
                else if (opcode_t'(instr_F2DEM1[6:0]) == OP_LOAD)
                    rs2_data_out = cache_read_data_M1;
                else
                    rs2_data_out = alu_out_EM1;   // MEM2 forwarding
            end
            4'b0011: begin //TODO: needs to change
                if (opcode_t'(instr_F2DEM1M2[6:0]) == OP_JAL | 
                    opcode_t'(instr_F2DEM1M2[6:0]) == OP_JALR) begin // jump instr
                    rs2_data_out = npc_plus4_F1F2DEM1M2;
                end
                else if (opcode_t'(instr_F2DEM1M2[6:0]) == OP_LOAD)
                    rs2_data_out = cache_read_data_M1M2;
                else
                    rs2_data_out = alu_out_EM1M2;   // MEM3 forwarding
            end
            4'd4: begin
                if (opcode_t'(instr_F2DEM1M2M3[6:0]) == OP_JAL | 
                    opcode_t'(instr_F2DEM1M2M3[6:0]) == OP_JALR) begin // jump instr
                    rs2_data_out = npc_plus4_F1F2DEM1M2M3;
                end
                else if (opcode_t'(instr_F2DEM1M2M3[6:0]) == OP_LOAD)
                    rs2_data_out = cache_read_data_M1M2M3;
                else
                    rs2_data_out = alu_out_EM1M2M3;   // MEM4 forwarding
            end
            4'd5: begin
                if (opcode_t'(instr_F2DEM1M2M3M4[6:0]) == OP_JAL | 
                    opcode_t'(instr_F2DEM1M2M3M4[6:0]) == OP_JALR) begin // jump instr
                    rs2_data_out = npc_plus4_F1F2DEM1M2M3M4;
                end
                else if (opcode_t'(instr_F2DEM1M2M3M4[6:0]) == OP_LOAD)
                    rs2_data_out = cache_read_data_M1M2M3M4;
                else
                    rs2_data_out = alu_out_EM1M2M3M4;   // MEM5 forwarding
            end
            4'd6: begin
                if (opcode_t'(instr_F2DEM1M2M3M4M5[6:0]) == OP_JAL | 
                    opcode_t'(instr_F2DEM1M2M3M4M5[6:0]) == OP_JALR) begin // jump instr
                    rs2_data_out = npc_plus4_F1F2DEM1M2M3M4M5;
                end
                else if (opcode_t'(instr_F2DEM1M2M3M4M5[6:0]) == OP_LOAD)
                    rs2_data_out = cache_read_data_M1M2M3M4M5;
                else
                    rs2_data_out = alu_out_EM1M2M3M4M5;   // MEM6 forwarding
            end
            4'd7: begin
                if (opcode_t'(instr_F2DEM1M2M3M4M5M6[6:0]) == OP_JAL | 
                    opcode_t'(instr_F2DEM1M2M3M4M5M6[6:0]) == OP_JALR) begin // jump instr
                    rs2_data_out = npc_plus4_F1F2DEM1M2M3M4M5M6;
                end
                else if (opcode_t'(instr_F2DEM1M2M3M4M5M6[6:0]) == OP_LOAD)
                    rs2_data_out = cache_read_data_M1M2M3M4M5M6;
                else
                    rs2_data_out = alu_out_EM1M2M3M4M5M6;   // MEM7 forwarding
            end
            4'd8: begin
                if (opcode_t'(instr_F2DEM1M2M3M4M5M6M7[6:0]) == OP_JAL | 
                    opcode_t'(instr_F2DEM1M2M3M4M5M6M7[6:0]) == OP_JALR) begin // jump instr
                    rs2_data_out = npc_plus4_F1F2DEM1M2M3M4M5M6M7; 
                end
                else if (opcode_t'(instr_F2DEM1M2M3M4M5M6M7[6:0]) == OP_LOAD)
                    rs2_data_out = cache_read_data_M1M2M3M4M5M6M7;
                else
                    rs2_data_out = alu_out_EM1M2M3M4M5M6M7;   // MEM8 forwarding
            end
            4'd9: begin // forward case if currently being written back to rf
                rs2_data_out = rd_data;
            end
            default: rs2_data_out = rs2_data; // no inferred latches
        endcase
    end

    // Forwarding Logic for rs1- when non-store instruction
    always_comb begin
        if (needToFwdAfterStall) alu_src1_forwarded = last_alu_src1_forwarded;
        else begin
        case (forwardA)
            4'b0000: alu_src1_forwarded = rs1_data; // Normal path (Register File Output)
            4'b0010: begin
                if (opcode_t'(instr_F2DE[6:0]) == OP_JAL | 
                    opcode_t'(instr_F2DE[6:0]) == OP_JALR) // jumps
                    alu_src1_forwarded = npc_plus4_F1F2DE;
                else // forwarding for non-jumping from M1 stage
            ...
            4'b0001: begin
                if (opcode_t'(instr_F2DEM1[6:0]) == OP_JAL | 
                    ...
                    alu_src1_forwarded = cache_read_data_M1;
                else // forwarding for non-jumping from M2 stage
                    alu_src1_forwarded = alu_out_EM1;
            end
            4'b0011: begin  // MEM2 forwarding
                if (opcode_t'(instr_F2DEM1M2[6:0]) == OP_JAL | 
                    opcode_t'(instr_F2DEM1M2[6:0]) == OP_JALR) // jumps
                    alu_src1_forwarded = npc_plus4_F1F2DEM1M2;
                else if (opcode_t'(instr_F2DEM1M2[6:0]) == OP_LOAD)
                    alu_src1_forwarded = cache_read_data_M1M2;
                else // forwarding for non-jumping from M3 stage
                    alu_src1_forwarded = alu_out_EM1M2;
            end
            4'd4: begin
                if (opcode_t'(instr_F2DEM1M2M3[6:0]) == OP_JAL | 
                    opcode_t'(instr_F2DEM1M2M3[6:0]) == OP_JALR) // jumps
                    alu_src1_forwarded = npc_plus4_F1F2DEM1M2M3;
                else if (opcode_t'(instr_F2DEM1M2M3[6:0]) == OP_LOAD)
                    alu_src1_forwarded = cache_read_data_M1M2M3;
                else // forwarding for non-jumping from M4 stage
                    alu_src1_forwarded = alu_out_EM1M2M3;
            end
            4'd5: begin
                if (opcode_t'(instr_F2DEM1M2M3M4[6:0]) == OP_JAL | 
                    opcode_t'(instr_F2DEM1M2M3M4[6:0]) == OP_JALR) // jumps
                    alu_src1_forwarded = npc_plus4_F1F2DEM1M2M3M4;
                else if (opcode_t'(instr_F2DEM1M2M3M4[6:0]) == OP_LOAD)
                    alu_src1_forwarded = cache_read_data_M1M2M3M4;
                else // forwarding for non-jumping from M5 stage
                    alu_src1_forwarded = alu_out_EM1M2M3M4;
            end
            4'd6: begin
                if (opcode_t'(instr_F2DEM1M2M3M4M5[6:0]) == OP_JAL | 
                    opcode_t'(instr_F2DEM1M2M3M4M5[6:0]) == OP_JALR) // jumps
                    alu_src1_forwarded = npc_plus4_F1F2DEM1M2M3M4M5;
                else if (opcode_t'(instr_F2DEM1M2M3M4M5[6:0]) == OP_LOAD)
                    alu_src1_forwarded = cache_read_data_M1M2M3M4M5;
                else // forwarding for non-jumping from M6 stage
                    alu_src1_forwarded = alu_out_EM1M2M3M4M5;
            end
            4'd7: begin
                if (opcode_t'(instr_F2DEM1M2M3M4M5M6[6:0]) == OP_JAL | 
                    opcode_t'(instr_F2DEM1M2M3M4M5M6[6:0]) == OP_JALR) // jumps
                    alu_src1_forwarded = npc_plus4_F1F2DEM1M2M3M4M5M6;
                else if (opcode_t'(instr_F2DEM1M2M3M4M5M6[6:0]) == OP_LOAD)
                    alu_src1_forwarded = cache_read_data_M1M2M3M4M5M6;
                else // forwarding for non-jumping from M7 stage
                    alu_src1_forwarded = alu_out_EM1M2M3M4M5M6;
            end
            4'd8: begin
                if (opcode_t'(instr_F2DEM1M2M3M4M5M6M7[6:0]) == OP_JAL | 
                    opcode_t'(instr_F2DEM1M2M3M4M5M6M7[6:0]) == OP_JALR) // jumps
                    alu_src1_forwarded = npc_plus4_F1F2DEM1M2M3M4M5M6M7;
                else if (opcode_t'(instr_F2DEM1M2M3M4M5M6M7[6:0]) == OP_LOAD)
                    alu_src1_forwarded = cache_read_data_M1M2M3M4M5M6M7;
                else // forwarding for non-jumping from M8 stage
                    alu_src1_forwarded = alu_out_EM1M2M3M4M5M6M7;
            end
            // forward case for data currently being written back to the register file
            4'd9: begin
                alu_src1_forwarded = rd_data;
            end
            default: alu_src1_forwarded = rs1_data; // no inferred latches
        endcase
        end
    end
    ...
    always_comb begin
        if (needToFwdBAfterStall) alu_src2_forwarded = last_alu_src2_forwarded;
        else begin
        case (forwardB)
            4'b0000: alu_src2_forwarded = rs2_data; // Normal path (Register File Output)
            4'b0010: begin 
                if (opcode_t'(instr_F2DE[6:0]) == OP_JAL | 
                    opcode_t'(instr_F2DE[6:0]) == OP_JALR) // jumps
                    alu_src2_forwarded = npc_plus4_F1F2DE;
                else // forwarding for non-jumping from M1 stage
                    alu_src2_forwarded = alu_out_E;
            end
            4'b0001: begin 
                if (opcode_t'(instr_F2DEM1[6:0]) == OP_JAL | 
                    opcode_t'(instr_F2DEM1[6:0]) == OP_JALR) // jumps
                    alu_src2_forwarded = npc_plus4_F1F2DEM1;
                else if (opcode_t'(instr_F2DEM1[6:0]) == OP_LOAD)
                    alu_src2_forwarded = cache_read_data_M1;
                else // forwarding for non-jumping from M2 stage
                    alu_src2_forwarded = alu_out_EM1;
            end
            4'b0011: begin
                if (opcode_t'(instr_F2DEM1M2[6:0]) == OP_JAL | 
                    opcode_t'(instr_F2DEM1M2[6:0]) == OP_JALR) // jumps
                    alu_src2_forwarded = npc_plus4_F1F2DEM1M2;
                else if (opcode_t'(instr_F2DEM1M2[6:0]) == OP_LOAD)
                    alu_src2_forwarded = cache_read_data_M1M2;
                else // forwarding for non-jumping from M3 stage
                    alu_src2_forwarded = alu_out_EM1M2;
            end
            4'd4: begin
                if (opcode_t'(instr_F2DEM1M2M3[6:0]) == OP_JAL | 
                    opcode_t'(instr_F2DEM1M2M3[6:0]) == OP_JALR) // jumps
                    alu_src2_forwarded = npc_plus4_F1F2DEM1M2M3;
                else if (opcode_t'(instr_F2DEM1M2M3[6:0]) == OP_LOAD)
                    alu_src2_forwarded = cache_read_data_M1M2M3;
                else // forwarding for non-jumping from M4 stage
                    alu_src2_forwarded = alu_out_EM1M2M3;
            end
            4'd5: begin
                if (opcode_t'(instr_F2DEM1M2M3M4[6:0]) == OP_JAL | 
                    opcode_t'(instr_F2DEM1M2M3M4[6:0]) == OP_JALR) // jumps
                    alu_src2_forwarded = npc_plus4_F1F2DEM1M2M3M4;
                else if (opcode_t'(instr_F2DEM1M2M3M4[6:0]) == OP_LOAD)
                    alu_src2_forwarded = cache_read_data_M1M2M3M4;
                else // forwarding for non-jumping from M5 stage
                    alu_src2_forwarded = alu_out_EM1M2M3M4;
            end
            4'd6: begin
                if (opcode_t'(instr_F2DEM1M2M3M4M5[6:0]) == OP_JAL | 
                    opcode_t'(instr_F2DEM1M2M3M4M5[6:0]) == OP_JALR) // jumps
                    alu_src2_forwarded = npc_plus4_F1F2DEM1M2M3M4M5;
                else if (opcode_t'(instr_F2DEM1M2M3M4M5[6:0]) == OP_LOAD)
                    alu_src2_forwarded = cache_read_data_M1M2M3M4M5;
                else // forwarding for non-jumping from M6 stage
                    alu_src2_forwarded = alu_out_EM1M2M3M4M5;
            end
            4'd7: begin
                if (opcode_t'(instr_F2DEM1M2M3M4M5M6[6:0]) == OP_JAL | 
                    opcode_t'(instr_F2DEM1M2M3M4M5M6[6:0]) == OP_JALR) // jumps
                    alu_src2_forwarded = npc_plus4_F1F2DEM1M2M3M4M5M6;
                else if (opcode_t'(instr_F2DEM1M2M3M4M5M6[6:0]) == OP_LOAD)
                    alu_src2_forwarded = cache_read_data_M1M2M3M4M5M6;
                else // forwarding for non-jumping from M7 stage
                    alu_src2_forwarded = alu_out_EM1M2M3M4M5M6;
            end
            4'd8: begin
                if (opcode_t'(instr_F2DEM1M2M3M4M5M6M7[6:0]) == OP_JAL | 
                    opcode_t'(instr_F2DEM1M2M3M4M5M6M7[6:0]) == OP_JALR) // jumps
                    alu_src2_forwarded = npc_plus4_F1F2DEM1M2M3M4M5M6M7;
                else if (opcode_t'(instr_F2DEM1M2M3M4M5M6M7[6:0]) == OP_LOAD)
                    alu_src2_forwarded = cache_read_data_M1M2M3M4M5M6M7;
                else // forwarding for non-jumping from M8 stage
                    alu_src2_forwarded = alu_out_EM1M2M3M4M5M6M7;
            end
            // forward case for data currently being written back to the register file
            4'd9: alu_src2_forwarded = rd_data; 
            default: alu_src2_forwarded = rs2_data; // no inferred latches
        endcase
        end
    end
        ...
        if (ctrl_signals.pc_source != PC_plus4)
            if (predScheme == 2'd1) begin // predicts always taken
                ...
                */
                ...
                if (bShouldBeTaken) begin // update btb data if branch is taken
                    sramWE = 1'b1;
                    if (ctrl_signals.pc_source == PC_cond && 
                        sramReadData[31:30] != 2'd3) begin // need to add to btb counter
                        updatedSramReadData = {pc[31:2], 
                            sramReadData[31:30] + 2'd1, npc_offset[31:2]};
                    end
                    else if (ctrl_signals.pc_source == PC_cond && 
                        sramReadData[31:30] == 2'd3) begin // do not need to add- will overflow
                ...
                else if (~bShouldBeTaken) begin // update btb when branch shouldn't be taken
                    sramWE = 1'b1;
                    if (ctrl_signals.pc_source == PC_cond && 
                        sramReadData[31:30] != 2'd0) begin // need to subtract from btb counter
                        updatedSramReadData = {pc[31:2], 
                            sramReadData[31:30] - 2'd1, npc_offset[31:2]};
                    end
                    else if (ctrl_signals.pc_source == PC_cond && 
                        sramReadData[31:30] == 2'd0) begin // do not need to sub- will underflow
                        updatedSramReadData = {pc[31:2], sramReadData[31:30], 
                            npc_offset[31:2]};
                    end
                    else if (ctrl_signals.pc_source == PC_uncond && 
                ...
                else begin // btb contents do not need to be updated
                    sramWE = 1'b0;
                    updatedSramReadData = 62'dx;
                end
            end
...
module mem
    (input logic clk, rst_l, sourcedFromMainMem, stallForCacheMiss1,
    input logic [31:0] instr, /*rs1_data, */rs2_data, alu_out, //pc,
    input ctrl_signals_t  ctrl_signals,
    input  logic [3:0][31:0] data_load,
    output logic data_load_en, cache_read_hit,
    output logic [31:0] data_store, cache_read_data,
    output logic [29:0] data_addr,
    output logic [3:0] data_store_mask,
    output logic stallForCacheMiss,
    output logic cache_read_miss, cache_rd_wr, cache_is_eviction, cache_enable);
    ...
    // cache logic variables - default params
    logic [29:0] cache_address;
    logic [3:0][31:0] cache_write_data;
    logic [3:0]                    cache_write_data_valid;
    logic [31:0] cache_read_dataTemp;
    logic cache_flush;   
    logic [1:0] boff;
    //logic ready;

    // cache combinational logic
    assign cache_address = alu_out[31:2]; // addr
    assign cache_enable = ctrl_signals.memRead || ctrl_signals.memWrite; // enable when rd/wr mem
    assign cache_rd_wr = ctrl_signals.memRead; // 1 for read, 0 for write
    assign cache_flush = 1'b0; // no flush for now
    assign boff = alu_out[3:2]; // block offset for writing 

    // L1 Cache
    cache447 #(.POLICY(0), .WAYS(1), .INDEX_BITS(3)) cache (.clk(clk), .rst_l(rst_l), .address(data_addr), 
                     .enable(cache_enable), .rd_wr(cache_rd_wr), 
                     .flush(cache_flush), 
                     .write_data_valid(cache_write_data_valid), 
                     .write_data(cache_write_data),
                     .read_data(cache_read_dataTemp), 
                     .read_hit(cache_read_hit), 
                     .read_miss(cache_read_miss), 
                     .is_eviction(cache_is_eviction));

    // if there is a cache miss- need to stall until the data is loaded from 
    // main memory
    always_comb begin
        if (cache_enable & cache_rd_wr & cache_read_miss & 
            (!stallForCacheMiss === 1'b1)) begin
            stallForCacheMiss = 1'b1;
        end
        else if (cache_enable & cache_rd_wr & cache_read_miss & 
            (stallForCacheMiss === 1'b1) & sourcedFromMainMem) begin
            stallForCacheMiss = 1'b0;
        end
        else stallForCacheMiss = 1'b0;
    end

    logic [31:0] mask;
    logic [3:0] inv_store_mask;
    // mask used for storing data
    assign inv_store_mask = ~data_store_mask;
    assign mask = {
        {8{inv_store_mask[3]}},
        {8{inv_store_mask[2]}},
        {8{inv_store_mask[1]}},
    ...
    // SW: cache_write_data and cache_write_valid logic
    always_comb begin
        //cache_write_data[0] = 32'd0;
        //cache_write_data[1] = 32'd0;
        //cache_write_data[2] = 32'd0;
        //cache_write_data[3] = 32'd0;
        //cache_write_data_valid = 4'd0;
        // if you are storing data use mask to ensure you are not overwriting 
        // anything you are not supposed to and set valid bit based on which 
        // word writing to
        if(data_store_mask != 4'd0) begin
            cache_write_data[0] = 32'd0;
            cache_write_data[1] = 32'd0;
            cache_write_data[2] = 32'd0;
            cache_write_data[3] = 32'd0;
            case (data_addr[1:0])
                2'd0: begin
                    cache_write_data[0] = data_store | (cache_read_dataTemp & mask);
                    cache_write_data_valid = 4'b0001;
                end
                2'd1: begin
                    cache_write_data[1] = data_store | (cache_read_dataTemp & mask);
                    cache_write_data_valid = 4'b0010;
                end
                2'd2: begin
                    cache_write_data[2] = data_store | (cache_read_dataTemp & mask);
                    cache_write_data_valid = 4'b0100;
                end
                2'd3: begin
                    cache_write_data[3] = data_store | (cache_read_dataTemp & mask);
                    cache_write_data_valid = 4'b1000;
                end
                default: begin
                    cache_write_data_valid = 4'b0000;
                end
            endcase
            // trying this too 
            //if (data_addr[1:0] != 2'd0) cache_write_data[0] = 32'd0;
            //if (data_addr[1:0] != 2'd1) cache_write_data[1] = 32'd0;
            //if (data_addr[1:0] != 2'd2) cache_write_data[2] = 32'd0;
            //if (data_addr[1:0] != 2'd3) cache_write_data[3] = 32'd0;
        end
        // if data has been loaded from main memory instead of reading you 
        // need to write this value back into the cache
        else if ((stallForCacheMiss === 1'b0) & (stallForCacheMiss1 === 1'b1) & 
            sourcedFromMainMem) begin
            cache_write_data[0] = 32'd0;
            cache_write_data[1] = 32'd0;
            cache_write_data[2] = 32'd0;
            cache_write_data[3] = 32'd0;
            cache_write_data[data_addr[1:0]] = data_load; // data loaded from main mem
            //trying
            //if (data_addr[1:0] != 2'd0) cache_write_data[0] = 32'd0;
            //if (data_addr[1:0] != 2'd1) cache_write_data[1] = 32'd0;
            //if (data_addr[1:0] != 2'd2) cache_write_data[2] = 32'd0;
            //if (data_addr[1:0] != 2'd3) cache_write_data[3] = 32'd0;
            case (data_addr[1:0]) // determine which word is valid based on last 2 bits
                2'd0: cache_write_data_valid = 4'b0001;
                2'd1: cache_write_data_valid = 4'b0010;
                2'd2: cache_write_data_valid = 4'b0100;
                2'd3: cache_write_data_valid = 4'b1000;
                default: cache_write_data_valid = 4'b0000;
            endcase
        end
        else begin
            cache_write_data_valid = 2'd0; // default write nothing
            cache_write_data[0] = 32'd0;
            cache_write_data[1] = 32'd0;
            cache_write_data[2] = 32'd0;
            cache_write_data[3] = 32'd0;
    ...
    // if there is a cache hit- then go ahead and determine the part of the 
    // word you want to write back to the register so if the value is forwarded 
    // it will be correct
    always_comb begin
                ...
                if (cache_read_hit) begin
                    unique case (ctrl_signals.ldst_mode)
                        LDST_W: begin // load full word
                            cache_read_data = cache_read_dataTemp;
                        end
                        LDST_H: begin // load half word (determined by alu_out)
                            if (alu_out[1] == 1'b0)
                                cache_read_data = 
                                    {{16{cache_read_dataTemp[15]}}, 
                                        cache_read_dataTemp[15:0]};
                            else 
                                cache_read_data = 
                                    {{16{cache_read_dataTemp[31]}}, 
                                        cache_read_dataTemp[31:16]};
                        end
                        LDST_HU: begin // load half word (unsigned)
                            if (alu_out[1] == 1'b0)
                                cache_read_data = {16'd0, 
                                    cache_read_dataTemp[15:0]};
                            else 
                                cache_read_data = {16'd0, 
                                    cache_read_dataTemp[31:16]};
                        ...
                        LDST_B: begin // load byte (which is determined by alu_out)
                            if (alu_out[1:0] == 2'd0)
                                cache_read_data = 
                                    {{24{cache_read_dataTemp[7]}}, 
                                        cache_read_dataTemp[7:0]};
                            else if (alu_out[1:0] == 2'd1)
                                cache_read_data = 
                                    {{24{cache_read_dataTemp[15]}}, 
                                        cache_read_dataTemp[15:8]};
                            else if (alu_out[1:0] == 2'd2)
                                cache_read_data = 
                                    {{24{cache_read_dataTemp[23]}}, 
                                        cache_read_dataTemp[23:16]};
                            else if (alu_out[1:0] == 2'd3)
                                cache_read_data = 
                                    {{24{cache_read_dataTemp[31]}}, 
                                        cache_read_dataTemp[31:24]};
                        end
                        LDST_BU: begin // load byte (unsigned)
                            if (alu_out[1:0] == 2'd0)
                                cache_read_data = {24'd0, 
                                    cache_read_dataTemp[7:0]};
                            else if (alu_out[1:0] == 2'd1)
                                cache_read_data = {24'd0, 
                                    cache_read_dataTemp[15:8]};
                        ...
                        default: cache_read_data = cache_read_dataTemp;
                    endcase
                end
                else cache_read_data = 32'dx;
            end
            default: cache_read_data = 32'dx;
        endcase
    end
...
module writeback
    (input logic /*clk, rst_l, */cache_hit,
    input ctrl_signals_t  ctrl_signals,
    input logic [31:0] npc_plus4, alu_out, instr, rs1_data, data_load, cache_read_data,
    input logic [4:0] rs1,
    ...
    // signals to pass back to the decode stage to writeback to the register file
    ...
    assign  rdToWrite              = instr[11:7];
        ...
        // file if a load operation- this happens only if there was a cache miss
        always_comb begin
        unique case (opcode_t'(instr[6:0]))
            OP_LOAD: begin // data_load_temp only matters if is a load
                if (cache_hit) begin
                    data_load_temp = cache_read_data;
                end 
                else begin
                unique case (ctrl_signals.ldst_mode)
                    LDST_W: begin // load full word
        ...
        case ({ctrl_signals.mem2RF, ctrl_signals.pc2RF}) 
            ...
            2'b01: begin
                if (opcode_t'(instr[6:0]) == OP_JAL | 
                    ...
                    rd_data = alu_out; // non-loads/jump instr
            end
            default: rd_data = alu_out; // non-loads/jump instr
        endcase
    end
