...
```bash
make verify TEST=benchmarks/mmmFpRV32I.c
```
...
For your convenience, here are the basic requirements for checkoff and final submission. Refer to the handout for full details.
All files should be at the top level of your directory.

If you wish to be graded with a different commit than you were checked off with, please let your checkoff TA know in an email the new commit hash and update it in your issue. **You will be graded based on the time of your final commit for the deadlines.**

#### Checkoff Requirements
We cannot check you off if you don't have all of these things!
* An issue with the information listed in the writeup
* Be ready to verbally answer the questions in the writeup
* `make autograde LAB_18447=3` for Lab 3 must pass
* Your design must synthesize and be latch free with -0.3 <= slack < 0.0 (and a clock period of 5.0 ns or less)
* Block diagram with critical path traced out and delays annotated, in a file titled **pipeline.pdf**
* Performance counters in a table, in a file titled **REPORT_perf.pdf**
* The timing report, **timing_riscv_core.rpt**
* Your synthesis log also needs to be available, **output/synthesis/synthesis.log**

Be ready to run the commands below for checkoff. 

* `git rev-parse HEAD`, to check the commit hash
* `git diff`, to check for any uncommitted changes
* `git status`, to check for any untracked files
* `make autograde LAB_18447=3`
* `grep -i latch output/synthesis/synthesis.log` to check for latches

#### Final Submission Requirements
* All files listed for checkoff requirements
* **REPORT_perf.pdf**, updated with full analysis
* **REPORT.pdf**, with the information detailed in the writeup

We will run private tests and autograde with only the files in `src` after the deadline. It is not a bad idea to reclone your repo and make sure everything works as expected. 
