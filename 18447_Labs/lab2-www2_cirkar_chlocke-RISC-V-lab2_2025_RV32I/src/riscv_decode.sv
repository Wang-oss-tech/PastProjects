...
module riscv_decode
    ...
    always_comb begin
        ...
        unique case (opcode)
            OP_OP: begin
                ctrl_signals.useImm=1'b0; // Use rs2_data
                ctrl_signals.rfWrite=1'b1; // Write result to register file
                ...
                unique case (rtype_funct3)
                    ...
                    // XOR 
                    FUNCT3_XOR: ctrl_signals.alu_op = ALU_XOR;

                    // AND
                    FUNCT3_AND: ctrl_signals.alu_op = ALU_AND;

                    // OR
                    FUNCT3_OR: ctrl_signals.alu_op = ALU_OR;

                    // SLT
                    FUNCT3_SLT: ctrl_signals.alu_op = ALU_SLT;

                    // SLTU
                    FUNCT3_SLTU: ctrl_signals.alu_op = ALU_SLTU;
                ...
                unique case (itype_int_funct3)
                    ...
                    FUNCT3_SRLI_SRAI: begin
                        unique case (funct7)
                            ...
                            default: ctrl_signals.alu_op = ALU_SRA;
                        endcase
        ...
        OP_AUIPC: begin
            ctrl_signals.useImm    = 1'b1;   
            ctrl_signals.rfWrite   = 1'b1; // writing to destination register     
            ctrl_signals.mem2RF    = 1'b0;      
            ...
            ctrl_signals.pc_source = PC_plus4;  // increment to size of next instruction
            ctrl_signals.ldst_mode = LDST_DC;
        end
        ...
        OP_STORE: begin
            ...
            // need to differentiate between instructions to determine
            // correct ctrl_signals.ldst_mode
            unique case (stype_funct3)
                ...
                default: ctrl_signals.illegal_instr = LDST_W;
            endcase
        end
